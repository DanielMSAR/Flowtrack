...
class LotesView(ctk.CTkFrame):
    def __init__(self, master, db_connection):
        ...
        self.lista_envases = []  # <-- NUEVO: Para guardar mapeos (id, nombre, capacidad) de la BD
    ...
    def _inicializar_datos_db(self):
        ...
        self._cargar_combo_envases()  # <-- NUEVO: Llena dinámicamente los envases desde MariaDB
    ...
    def _crear_formulario_envasado(self):
        ...
        # CORREGIDO: El combo se inicializa vacío y en readonly. Se cargará dinámicamente desde la BD.
        self.cmb_envase = ctk.CTkComboBox(self.tab_envasado, values=[], width=180, state="readonly")
    ...
    def _cargar_combo_envases(self):
        ...
        self.lista_envases = []
        try:
            # Reemplaza 'envases' por el nombre real de tu tabla si difiere
            ...
            if resultados:
                for fila in resultados:
                    ...
                    # Formato del texto visible en el combo: "ID - Nombre (Capacidad kg)"
                    valores.append(f"{id_env} - {nombre} ({cap} kg)")
            
            if valores:
                self.cmb_envase.configure(values=valores)
                self.cmb_envase.set(valores[0])  # Preselecciona el primero por defecto
            else:
    ...
    def _on_lote_seleccionado(self, event):
        ...
        if res:
            ...
            # --- NUEVO: Seteo del tipo de envase guardado en el combo ---
            if id_en:
                encontrado = False
    ...
    def _guardar_envasado(self):
        ...
        # --- NUEVO: Obtener idenvase real a partir del combo ---
        envase_sel = self.cmb_envase.get()
        try:
            # Parseamos el ID separándolo por el guion
        ...
        # Se reemplaza el idenvase = 1 rígido por %s dinámico
        query = """UPDATE lotes SET kgsenv = %s, fechaenv = %s, 
                operarioenv = 101, idenvase = %s, envasado = %s WHERE id = %s"""
        self.db.execute_non_query(query, (int(kgs_env), fecha_sql, id_envase, is_env, id_lote))
