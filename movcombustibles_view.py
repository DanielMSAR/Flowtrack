...
class MovCombustiblesView(ctk.CTkFrame):
    def __init__(self, master, db_connection):
        ...
        self.combustibles_dict = {}
        self.vehiculos_dict = {}

        # --- CONTENEDOR DE PESTAÑAS ---
        self.tabview = ctk.CTkTabview(self, segmented_button_selected_color="#7a8754", segmented_button_selected_hover_color="#636e43", text_color="white")
        ...
        self.cargar_combos_combustibles()
    ...
    def _configurar_tab_stock(self):
        ...
        # -----------------------------------------------------------------
        # MITAD SUPERIOR: PANEL DE MAESTRO (STOCK ACTUAL DE COMBUSTIBLES)
        # -----------------------------------------------------------------
        ...
        ctk.CTkLabel(self.header_superior, text="📋 ESTADO DE TANQUES / COMBUSTIBLE CONSOLIDADO", font=("Arial", 12, "bold"), text_color="#4d5433").pack(side="left", anchor="w")
        ...
        columnas_stock = ("idarticulo", "combustible", "cantidad", "puntocritico")
        ...
        self.tree_stock.heading("idarticulo", text="ID COMBUSTIBLE")
        self.tree_stock.heading("combustible", text="TIPO DE COMBUSTIBLE")
        self.tree_stock.heading("cantidad", text="LITROS ACTUALES")
        self.tree_stock.heading("puntocritico", text="PUNTO CRÍTICO")

        self.tree_stock.column("idarticulo", width=120, anchor="center")
        self.tree_stock.column("combustible", width=330, anchor="w")
        ...
        self.tree_stock.bind("<<TreeviewSelect>>", self._al_seleccionar_combustible)

        # -----------------------------------------------------------------
        # MITAD INFERIOR: PANEL DE DETALLE (HISTORIAL DE VALES Y CARGAS)
        # -----------------------------------------------------------------
        ...
        self.lbl_historial_titulo = ctk.CTkLabel(self.header_inferior, text="📜 HISTORIAL DE MOVIMIENTOS: (Seleccione un combustible)", font=("Arial", 12, "bold"), text_color="#333333")
        ...
        self.btn_imp_detalle = ctk.CTkButton(
            ...
            text_color="white", font=("Arial", 11, "bold"), width=160, height=28, state="disabled", command=self.imprimir_historial_combustible
        )
        ...
        self.tree_movs.heading("agrega", text="ENTRADA (Lts)")
        self.tree_movs.heading("descuenta", text="DESPACHO (Lts)")
        self.tree_movs.heading("final", text="ST. FINAL")
        self.tree_movs.heading("comp", text="Nº VALE / REMITO")
        self.tree_movs.heading("empleado", text="RESPONSABLE")
    ...
    def cargar_grid_stock(self):
        ...
        self.lbl_historial_titulo.configure(text="📜 HISTORIAL DE MOVIMIENTOS: (Seleccione un combustible)")
        ...
        query = """
            SELECT combustibles.id, combustibles.combustible, IFNULL(stockcombustibles.cantidad, 0.0) AS cantidad, combustibles.puntocritico 
            FROM combustibles 
            LEFT JOIN stockcombustibles ON combustibles.id = stockcombustibles.idarticulo
            ORDER BY combustibles.combustible ASC
        """
    ...
    def _al_seleccionar_combustible(self, event):
        ...
        id_combustible = valores[0]
        nombre_combustible = valores[1]

        self.lbl_historial_titulo.configure(text=f"📜 HISTORIAL DE MOVIMIENTOS: {nombre_combustible.upper()} (ID: {id_combustible})")
        ...
        query_movs = """
            ...
            FROM regmovcombustibles m
            LEFT JOIN vehiculos v ON m.idvehiculo = v.idvehiculo
            WHERE m.idcombustible = %s
            ORDER BY m.fecha DESC
        """
        historial = self.db.execute_query(query_movs, (id_combustible,))
        ...
        self.btn_detalle_activo = {"id": id_combustible, "nombre": nombre_combustible}
        self.btn_imp_detalle.configure(state="normal", fg_color="#4d5433", hover_color="#3b4026")

    # =====================================================================
    # PESTAÑA: INGRESOS (DESCARGA DE CAMIÓN CISTERNA A TANQUES)
    # =====================================================================
    def _configurar_tab_ingresos(self):
        ...
        self.ingresos_container.pack(fill="none", expand=True, padx=50, pady=10)

        ctk.CTkLabel(self.ingresos_container, text="REGISTRO DE INGRESO DE COMBUSTIBLE (CISTERNA)", font=("Arial", 16, "bold"), text_color="#4d5433").grid(row=0, column=0, columnspan=2, pady=12, padx=30, sticky="w")

        # Combo Combustibles
        ctk.CTkLabel(self.ingresos_container, text="Seleccione Tipo Combustible:", font=("Arial", 12, "bold"), text_color="black").grid(row=1, column=0, padx=30, pady=(5, 2), sticky="w")
        self.cmb_combustibles = ctk.CTkComboBox(self.ingresos_container, width=320, height=35, values=[], command=self._actualizar_lbl_stock_inicial)
        self.cmb_combustibles.grid(row=2, column=0, padx=30, pady=(0, 10), sticky="w")
        self.cmb_combustibles.set("Seleccione un combustible...")

        self.lbl_stock_previo = ctk.CTkLabel(self.ingresos_container, text="Litros Actuales en Tanque: --", font=("Arial", 12, "italic"), text_color="blue")
        self.lbl_stock_previo.grid(row=2, column=1, padx=10, pady=(0, 10), sticky="w")

        # Combo Vehículos (Camión del proveedor / Fletero si aplica)
        ctk.CTkLabel(self.ingresos_container, text="Vehículo / Cisterna Prov:", font=("Arial", 12, "bold"), text_color="black").grid(row=3, column=0, padx=30, pady=(5, 2), sticky="w")
        ...
        # Cuadro de texto para Nombre del Chofer / Proveedor
        ctk.CTkLabel(self.ingresos_container, text="Descargado por (Nombre de Persona):", font=("Arial", 12, "bold"), text_color="black").grid(row=5, column=0, padx=30, pady=(5, 2), sticky="w")
        self.ent_empleado_ingreso = ctk.CTkEntry(self.ingresos_container, width=320, height=35, font=("Arial", 13), placeholder_text="Nombre del chofer o proveedor...")
        self.ent_empleado_ingreso.grid(row=6, column=0, padx=30, pady=(0, 10), sticky="w")

        # Cantidad de Litros
        ctk.CTkLabel(self.ingresos_container, text="Litros a Ingresar:", font=("Arial", 12, "bold"), text_color="black").grid(row=7, column=0, padx=30, pady=(5, 2), sticky="w")
        ...
        # Remito / Factura
        ctk.CTkLabel(self.ingresos_container, text="Número de Remito / Comprobante:", font=("Arial", 12, "bold"), text_color="black").grid(row=9, column=0, padx=30, pady=(5, 2), sticky="w")
        ...
        self.btn_guardar_ingreso = ctk.CTkButton(
            self.ingresos_container, text="PROCESAR DESCARGA / INGRESO", fg_color="#4d5433", hover_color="#3b4026",
    ...
    def _actualizar_lbl_stock_inicial(self, valor_combo):
        id_combustible = self.combustibles_dict.get(valor_combo)
        if id_combustible:
            stock_actual = self._obtener_stock_actual_db(id_combustible)
            self.lbl_stock_previo.configure(text=f"Litros Actuales en Tanque: {stock_actual:.2f} Lts")

    def _obtener_stock_actual_db(self, id_combustible):
        query = "SELECT cantidad FROM stockcombustibles WHERE idarticulo = %s"
        resultado = self.db.execute_query(query, (id_combustible,))
        return float(resultado[0][0]) if resultado and resultado[0][0] is not None else 0.0

    def guardar_ingreso(self):
        comb_sel = self.cmb_combustibles.get()
        vehiculo_sel = self.cmb_vehiculos.get()
        empleado_tipeado = self.ent_empleado_ingreso.get().strip().upper()
        cantidad_raw = self.ent_cantidad.get().strip()
        comprobante = self.ent_comp.get().strip()

        if not comb_sel or comb_sel == "Seleccione un combustible...":
            messagebox.showwarning("Atención", "Debe seleccionar un combustible válido de la lista.")
            return
        ...
        try:
            ...
            messagebox.showerror("Error", "La cantidad de litros debe ser un valor numérico mayor a cero.")
            return

        responsable_final = empleado_tipeado if empleado_tipeado else self.current_user_name
        id_combustible = self.combustibles_dict.get(comb_sel)
        id_vehiculo = self.vehiculos_dict.get(vehiculo_sel) if (vehiculo_sel and vehiculo_sel != "Seleccione un Vehiculo...") else None

        stock_inicial = self._obtener_stock_actual_db(id_combustible)
        stock_final = stock_inicial + cantidad_agrega

        try:
            # 1. Registro de movimiento histórico
            query_mov = """
                INSERT INTO regmovcombustibles (detalle, fecha, stocki, agrega, descuenta, stockf, comp, idcombustible, empleado, idvehiculo)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            params_mov = (comb_sel, datetime.now(), stock_inicial, cantidad_agrega, 0.0, stock_final, comprobante, id_combustible, responsable_final, id_vehiculo)
            self.db.execute_query(query_mov, params_mov)

            # 2. Actualización / Inserción en la tabla de existencias consolidadas
            chk = self.db.execute_query("SELECT idstock FROM stockcombustibles WHERE idarticulo = %s", (id_combustible,))
            
            if chk:
                query_stock = "UPDATE stockcombustibles SET cantidad = %s WHERE idarticulo = %s"
                params_stock = (stock_final, id_combustible)
            else:
                query_stock = "INSERT INTO stockcombustibles (idarticulo, cantidad) VALUES (%s, %s)"
                params_stock = (id_combustible, stock_final)
            ...
            messagebox.showinfo("Éxito", f"Ingreso a tanque procesado correctamente.\nStock final: {stock_final:.2f} Lts")
            ...
            self.cmb_combustibles.set("Seleccione un combustible...")
            self.cmb_vehiculos.set("Seleccione un Vehiculo...")
            self.lbl_stock_previo.configure(text="Litros Actuales en Tanque: --")
            self.cargar_grid_stock()
            ...
            messagebox.showerror("Error", f"No se pudo completar el ingreso a tanques:\n{e}")

    # =====================================================================
    # PESTAÑA: SALIDAS (DESPACHO DE SURTIDOR INTERNO A MAQUINARIAS)
    # =====================================================================
    def _configurar_tab_salidas(self):
        ...
        self.salidas_container.pack(fill="none", expand=True, padx=50, pady=10)

        ctk.CTkLabel(self.salidas_container, text="REGISTRO DE DESPACHO / CONSUMO DE SURTIDOR", font=("Arial", 16, "bold"), text_color="#bc4749").grid(row=0, column=0, columnspan=2, pady=12, padx=30, sticky="w")

        # Combo Combustibles para Salida
        ctk.CTkLabel(self.salidas_container, text="Seleccione Tipo Combustible a Despachar:", font=("Arial", 12, "bold"), text_color="black").grid(row=1, column=0, padx=30, pady=(5, 2), sticky="w")
        self.cmb_combustibles_salida = ctk.CTkComboBox(self.salidas_container, width=320, height=35, values=[], command=self._actualizar_lbl_stock_inicial_salida)
        self.cmb_combustibles_salida.grid(row=2, column=0, padx=30, pady=(0, 10), sticky="w")
        self.cmb_combustibles_salida.set("Seleccione un combustible...")

        self.lbl_stock_previo_salida = ctk.CTkLabel(self.salidas_container, text="Litros Actuales en Tanque: --", font=("Arial", 12, "italic"), text_color="red")
        self.lbl_stock_previo_salida.grid(row=2, column=1, padx=10, pady=(0, 10), sticky="w")

        # Combo Vehículos / Destino para Salida
        ctk.CTkLabel(self.salidas_container, text="Vehículo / Máquina que carga:", font=("Arial", 12, "bold"), text_color="black").grid(row=3, column=0, padx=30, pady=(5, 2), sticky="w")
        ...
        self.cmb_vehiculos_salida.set("Seleccione un Vehiculo...")

        # Cuadro de texto para Nombre del Operario / Tractorista
        ...
        self.ent_empleado_salida = ctk.CTkEntry(self.salidas_container, width=320, height=35, font=("Arial", 13), placeholder_text="Nombre de quien retira / carga...")
        self.ent_empleado_salida.grid(row=6, column=0, padx=30, pady=(0, 10), sticky="w")

        # Litros a Despachar
        ctk.CTkLabel(self.salidas_container, text="Litros Despachados:", font=("Arial", 12, "bold"), text_color="black").grid(row=7, column=0, padx=30, pady=(5, 2), sticky="w")
        ...
        ctk.CTkLabel(self.salidas_container, text="Nº de Vale Interno (Opcional):", font=("Arial", 12, "bold"), text_color="black").grid(row=9, column=0, padx=30, pady=(5, 2), sticky="w")
        ...
        self.btn_guardar_salida = ctk.CTkButton(
            self.salidas_container, text="PROCESAR VALE / DESPACHO", fg_color="#bc4749", hover_color="#9b2226",
    ...
    def _actualizar_lbl_stock_inicial_salida(self, valor_combo):
        ...
        if id_combustible:
            stock_actual = self._obtener_stock_actual_db(id_combustible)
            self.lbl_stock_previo_salida.configure(text=f"Litros Actuales en Tanque: {stock_actual:.2f} Lts")

    def guardar_salida(self):
        comb_sel = self.cmb_combustibles_salida.get()
        vehiculo_sel = self.cmb_vehiculos_salida.get()
        empleado_tipeado = self.ent_empleado_salida.get().strip().upper()
        ...
        try:
            ...
            messagebox.showerror("Error", "La cantidad de litros a descontar debe ser mayor a cero.")
            return
        ...
        if cantidad_descuenta > stock_inicial:
            messagebox.showwarning("Existencias Insuficientes", f"Tanque sin capacidad operativa.\n\nLitros disponibles: {stock_inicial:.2f}\nCantidad solicitada: {cantidad_descuenta:.2f}")
            return
        ...
        try:
            query_mov = """
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            params_mov = (comb_sel, datetime.now(), stock_inicial, 0.0, cantidad_descuenta, stock_final, comprobante, id_combustible, responsable_final, id_vehiculo)
            self.db.execute_query(query_mov, params_mov)

            self.db.execute_query("UPDATE stockcombustibles SET cantidad = %s WHERE idarticulo = %s", (stock_final, id_combustible))
            ...
            messagebox.showinfo("Éxito", f"Despacho de combustible procesado correctamente.\nLitros remanentes en tanque: {stock_final:.2f}")
            ...
            self.cmb_combustibles_salida.set("Seleccione un combustible...")
            self.cmb_vehiculos_salida.set("Seleccione un Vehiculo...")
            self.lbl_stock_previo_salida.configure(text="Litros Actuales en Tanque: --")
            self.cargar_grid_stock()
            ...
            if hasattr(self.db, "connection") and self.db.connection:
                ...
                self.db.conn.rollback()
            messagebox.showerror("Error", f"No se pudo completar la transacción del surtidor:\n{e}")

    # =====================================================================
    # CONTROL DE DATOS Y CARGA GENERAL DE COMBOS (TABLAS DE MAESTROS)
    # =====================================================================
    def cargar_combos_combustibles(self):
        res_combustibles = self.db.execute_query("SELECT id, combustible FROM combustibles ORDER BY combustible ASC")
        if res_combustibles:
            self.combustibles_dict = {r[1]: r[0] for r in res_combustibles}
            self.cmb_combustibles.configure(values=list(self.combustibles_dict.keys()))
            self.cmb_combustibles_salida.configure(values=list(self.combustibles_dict.keys()))
    ...
    # =====================================================================
    # LÓGICA DE EMISIÓN DE REPORTES EN PDF (A4 - REPORTLAB)
    # =====================================================================
    def imprimir_stock_completo(self):
        try:
            ...
            ruta_pdf = os.path.join(carpeta_reportes, f"stock_combustibles_{fecha_str}.pdf")
            ...
            story.append(Paragraph(f"<b>Reporte:</b> Estado Consolidado de Tanques de Combustible<br/><b>Fecha de emisión:</b> {datetime.now().strftime('%d/%m/%Y %H:%M')}<br/><b>Operario:</b> {self.current_user_name}", style_subtitulo))
            story.append(Spacer(1, 10))

            items = self.tree_stock.get_children()
            if not items:
                messagebox.showwarning("Reporte Vacío", "No hay datos de tanques para exportar.")
                return

            data_tabla = [[
                Paragraph("<b>ID COMBUSTIBLE</b>", style_celda_header),
                Paragraph("<b>TIPO DE COMBUSTIBLE</b>", style_celda_header),
                Paragraph("<b>LITROS ACTUALES</b>", style_celda_header),
            ...
            tabla_reporte = Table(data_tabla, colWidths=[100, 230, 95, 90])
            ...
            messagebox.showerror("Error de Impresión", f"Ocurrió un error al generar el PDF:\n{e}")

    def imprimir_historial_combustible(self):
        ...
        try:
            ...
            fecha_str = datetime.now().strftime("%Y%m%d_%H%M%S")
            ruta_pdf = os.path.join(carpeta_reportes, f"historial_combustible_{id_sel}_{fecha_str}.pdf")
            ...
            story.append(Paragraph("FLOWTRACK - AUDITORÍA DE MAQUINARIAS Y VALES", style_titulo))
            story.append(Paragraph(f"<b>Combustible Consultado:</b> {nom_sel.upper()} (ID Sistema: {id_sel})<br/><b>Fecha de Reporte:</b> {datetime.now().strftime('%d/%m/%Y %H:%M')}<br/><b>Emitido por:</b> {self.current_user_name}", style_subtitulo))
            story.append(Spacer(1, 10))

            items_movs = self.tree_movs.get_children()
            if not items_movs:
                messagebox.showwarning("Atención", "No se registran despachos históricos para exportar.")
                return
            ...
            data_tabla = [[
                ...
                Paragraph("<b>ENTRADA (Lts)</b>", style_celda_header),
                Paragraph("<b>DESPACHO (Lts)</b>", style_celda_header),
                Paragraph("<b>ST. FINAL</b>", style_celda_header),
                Paragraph("<b>Nº VALE / REMITO</b>", style_celda_header),
                Paragraph("<b>RESPONSABLE</b>", style_celda_header),
